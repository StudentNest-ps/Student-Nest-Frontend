...
import { Clock, CheckCircle, XCircle, LayoutGrid, TableIcon, Home } from "lucide-react"
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
          ...
          <motion.div variants={fadeInUp} className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <Card className="bg-background/50 backdrop-blur-sm border-yellow-500/20 shadow-lg shadow-yellow-500/10">
              <CardContent className="p-4 text-center">
                ...
                <div className="text-2xl font-bold text-headline">{stats.pending}</div>
            ...
            <Card className="bg-background/50 backdrop-blur-sm border-green-500/20 shadow-lg shadow-green-500/10">
              <CardContent className="p-4 text-center">
                ...
                <div className="text-2xl font-bold text-headline">{stats.accepted}</div>
                <div className="text-sm text-muted-foreground">Accepted</div>
              </CardContent>
            </Card>

            <Card className="bg-background/50 backdrop-blur-sm border-red-500/20 shadow-lg shadow-red-500/10">
              <CardContent className="p-4 text-center">
                <div className="flex items-center justify-center mb-2">
                  <XCircle className="h-5 w-5 text-red-500" />
                </div>
                <div className="text-2xl font-bold text-headline">{stats.rejected}</div>
        ...
        <AnimatePresence mode="wait">
          {isLoading ? (
            ...
            >
              {viewMode === "cards" ? <BookingCard bookings={bookings} /> : <BookingTable bookings={bookings} />}
            </motion.div>
          )}
        </AnimatePresence>
      </div>
